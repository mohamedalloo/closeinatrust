/* CloseInATrust match form — one big friendly question per screen.
...
const answers = {
  goal: "", property: "", amount: "", income: "", docs: "", credit: "",
};
...
const MATCH_RULES = [
  { match: (a) => a.income === "rentals" || a.property === "investment", name: "DSCR Investor Loan (entity vesting)" },
  { match: (a) => a.income === "assets", name: "Asset-Based Loan (trust vesting)" },
  { match: (a) => a.income === "business", name: "Bank Statement Loan (entity vesting)" },
  { match: () => true, name: "Flexible-Doc Loan (entity vesting)" },
];
function matchedProduct() {
  return MATCH_RULES.find((r) => r.match(answers)).name + " — CloseInATrust";
}
...
const STEPS = [
  {
    ...
    choices: [
      { value: "buy-trust", label: "Buy a home in my trust", sub: "Close with the trust on title from day one" },
      { value: "refi-trust", label: "Refinance a home that's in a trust", sub: "Without taking it out of the trust" },
      { value: "llc", label: "Buy or refinance in an LLC", sub: "Rentals, flips, and portfolio properties" },
      { value: "explore", label: "Still figuring out vesting", sub: "Trust vs. LLC vs. personal — get straight answers" },
    ],
  },
  {
    key: "property",
    ...
    choices: [
      { value: "investment", label: "A rental or investment property" },
      { value: "primary", label: "The home I live in" },
      { value: "second", label: "A second home" },
    ],
  },
  {
    key: "amount",
    question: "Roughly what is the property worth?",
  ...
  {
    key: "income",
    question: "How would this loan qualify?",
    help: "Pick the closest fit — specialists can combine several.",
    choices: [
      { value: "rentals", label: "The property's rent", sub: "It pays for itself — or will" },
      { value: "business", label: "My business income", sub: "Self-employed, deposits tell the story" },
      { value: "assets", label: "Savings & investments", sub: "Asset-based qualification" },
      { value: "working", label: "A regular paycheck", sub: "W-2 income" },
    ],
  },
  {
    key: "credit",
...
function renderChoice(step) {
  ...
  formBody.querySelectorAll(".choice-btn").forEach((btn) =>
    btn.addEventListener("click", () => {
      ...
      if (step.key === "income") answers.docs = btn.dataset.value === "assets" ? "assets" : "";
      stepIndex++;
      renderStep();
    })
  );
